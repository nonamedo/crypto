...
{
    internal class WithdrawGasCommand: ICommand
    {
        private readonly ICryptoService _service;
        private readonly IInputOutput _io;

        public WithdrawGasCommand(ICryptoService service, IInputOutput io)
        {
            _service = service;
            _io = io;
        }
        ...
        public void Execute()
        {
            ...
            var txid = _service.WithdrawGasAsync(
                from: from,
                toAddress: toAddress).GetAwaiter().GetResult();
